...
func build_mesh(grid_cells: Vector2, grid_size: Vector2, color:Color = Color.white) -> Mesh:
	_initialize_grid(color);
	_build_grid(grid_cells, grid_size);
	_finalize_grid();
...
func _build_grid(grid_cells: Vector2, grid_size: Vector2):
	for x in range(grid_cells.x):
		for y in range(grid_cells.y):
			_build_grid_cell(Vector2(x, y), grid_cells, grid_size);


func _build_grid_cell(cell_position: Vector2, grid_cells: Vector2, grid_size: Vector2):
	_add_grid_cell_indices()
	_add_grid_cell_vertices(cell_position, grid_cells, grid_size);


func _add_grid_cell_vertices(cell_position: Vector2, grid_cells: Vector2, grid_size: Vector2):
	for offset in square_offsets:
		...
		var vertex_position = grid_size * position_normalized;
		var vertex = Vector3(vertex_position.x, vertex_position.y, 0);
