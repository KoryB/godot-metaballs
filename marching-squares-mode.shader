...
void fragment() {
	...
	float influence = calculate_ball_influence(v_local_space, circle_position, circle_radius_squared);
	      influence += calculate_ball_influence(v_local_space, circle_position + vec2(0, 0.5*sin(TIME)), circle_radius_squared);
	      influence += calculate_ball_influence(v_local_space, circle_position + vec2(0.5*cos(2.0*TIME + 3.14), 0), circle_radius_squared);
